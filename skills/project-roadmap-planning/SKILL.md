...
description: "项目开发路线图规划。基于产品概述和模块依赖，规划功能的开发顺序和里程碑。当用户说'规划路线图'、'开发顺序'、'里程碑规划'、'roadmap'时触发。"
---
...
5.  **🔴 CHECKPOINT**：展示里程碑规划和依赖关系图，等用户确认后再生成文档。
6.  **生成文档**：输出符合模板的 Markdown 文档，**根据检测结果预先勾选已完成的任务**。

7.  **最终交付**：保存到 `specs/开发路线图.md`。
...
| `specs/项目结构.md` 不存在 | 可继续但标注"模块划分参考不完整"，建议先运行 `project-structure` |
| 模块间存在循环依赖 | 标注循环依赖链，建议用户重构模块边界或提取公共模块 |
| MVP 范围过大（超过总功能的 60%） | 提示精简 MVP，建议将部分功能移到 Milestone 2 |
...
| 反模式 | 为什么不要做 |
|--------|-------------|
| "所有功能并行开发" | 模块间有依赖关系，地基没打好就建上层会返工 |
| "MVP 包含所有功能" | MVP 应只含核心闭环，功能堆砌拖慢交付节奏 |
| "不检测现有进度直接规划" | 已完成的模块应跳过，避免重复劳动 |
| "路线图一旦定就不能改" | 路线图是指导而非束缚，开发中发现新依赖应调整 |
