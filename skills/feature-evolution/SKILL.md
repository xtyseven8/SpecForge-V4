...
- **目录存在且包含 `.html` 文件** → 读取所有 html 原型文档。变更涉及 UI 时，必须同步评估原型是否需要更新：
  - **变更影响 UI**：如果变更会改变页面样式、布局、交互，必须同步更新对应的原型 html 文件，保持原型与实现一致
  - **变更不影响 UI**：纯后端/逻辑变更无需更新原型，但在影响分析中明确说明"本变更不涉及 UI，原型无需更新"
  - **原型更新原则**：就地修改原型 html，不破坏原型的整体结构；同时在变更日志中记录原型变更
...
**UI/原型层面**（四份文档驱动模式启用时）：变更是否影响页面样式、布局、交互？`specs/prototype/` 下的原型 html 是否需要同步更新？哪些原型元素需要修改？

**代码层面**（如果已有代码）：哪些现有文件需要改动？需要新增哪些文件？已有的测试会不会受影响？

**任务层面**（如果开发进行中）：已完成的任务是否受影响？还未执行的任务是否需要调整？变更是插入到现有任务序列中，还是追加在后面？

分析完后，向用户展示影响范围。**🔴 CHECKPOINT**：这是整个流程中最重要的确认点——影响范围对了，后面的文档更新和任务规划才不会跑偏。**必须**等用户确认影响范围后再继续。
...
- 改了核心逻辑 → 三份文档可能都要动
- **涉及 UI 变化**（四份文档驱动模式）→ 需求/技术方案 + 原型 html 都要同步更新，保持原型与实现一致
...
**变更涉及 UI 或核心逻辑时，必须包含功能测试回归任务**——变更完成后重新调用 `feature-functional-testing`，重点测试：
- 变更点本身的新 AC（编写新的测试用例）
- 变更可能影响的原有功能（基于已有测试用例回归）
- 原型一致性（如变更涉及 UI，对照更新后的原型）

在增量任务计划末尾追加：
```
Task-{N}: 功能测试回归
- 验证标准: 调用 feature-functional-testing Skill，覆盖变更点 + 受影响的原有功能
- AC 关联: 新增 AC + 可能受影响的旧 AC
- 依赖: 前置任务全部完成且通过代码审查
```
...
- 每份增量任务计划必须包含回归验证任务
- **变更涉及 UI 或核心逻辑时，必须包含功能测试回归任务**（调用 feature-functional-testing）
- 变更范围过大时（超过原功能 70%），建议走新功能流程而不是硬做增量
- **四份文档驱动模式启用时**：涉及 UI 变化的变更必须同步更新原型 html，保持原型与实现一致；不涉及 UI 的变更必须在影响分析中明确说明"原型无需更新"

---

# Red Flags

以下想法意味着你在合理化跳过变更影响分析——停下来：

| 想法 | 现实 |
|------|------|
| "这个变更很小，不用做影响分析" | 小变更也可能影响现有功能，影响分析是必须的 |
| "直接改文档就行了，不用追加变更日志" | 变更日志是追溯变更历史的依据，不能省 |
| "已完成的任务改一下也没关系" | 已通过验证的任务和代码不得被删除或重写 |
| "回归验证任务可以省略" | 每份增量任务计划必须包含回归验证任务 |
| "变更范围超过 70%，但硬做也行" | 超过 70% 建议走新功能流程，硬做增量成本更高 |
| "CR 编号随便定一个就行" | CR 编号和任务编号在同一功能内连续递增，不能乱 |
| "新 AC 用什么格式无所谓" | 新增/修改的 AC 必须使用 Given-When-Then 格式 |
| "UI 变了但原型不用更新" | 四份文档驱动模式下，UI 变更必须同步更新原型，否则原型失去参考价值 |
| "原型更新太麻烦，先改代码再说" | 原型是 UI 真相源，必须先更新原型再改代码，否则实现会偏离设计 |
| "变更改完了，代码审查过了就行" | 变更涉及 UI 或核心逻辑时，必须重新调用 feature-functional-testing 做回归测试，验证变更没有破坏原有功能 |
