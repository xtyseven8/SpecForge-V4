...
- **目录存在且包含 `.html` 文件** → 读取所有 html 原型文档。拆任务时必须：
  - **UI 任务的验证标准包含原型一致性检查点**：涉及 UI 的任务，验证标准中必须包含"对照原型 `specs/prototype/xxx.html` 验证样式/布局/排版一致"
  - **识别原型中的可复用组件**：原型中重复出现的 UI 模式应拆为独立的组件任务，优先实现
...
其他情况下，直接给出你的判断。拆完后展示任务总数和预估总工时，**🔴 CHECKPOINT**：等用户确认后再生成文档。
...
- **四份文档驱动模式启用时**（`specs/prototype/` 非空）：涉及 UI 的任务验证标准必须包含原型一致性检查点，UI 任务必须标注对应的原型文件路径

---

# Red Flags

以下想法意味着你在合理化降低任务规划质量——停下来：

| 想法 | 现实 |
|------|------|
| "验证标准写'返回正确数据'就行了" | 模糊的验证标准无法驱动 TDD，必须具体到输入和预期输出 |
| "这条 AC 没有对应任务，应该不重要" | 每条 AC 必须能追溯到至少一个任务，遗漏就是缺口 |
| "测试可以单独拆一个任务" | 测试不是独立任务，每个任务在 TDD 的 RED 阶段完成测试 |
| "这个任务粒度大一点没关系" | 任务应原子化，30 分钟到 2 小时之间，太大无法验证 |
| "水平切片更简单，按技术层拆吧" | 默认垂直切片，水平切片会导致无法增量验证 |
| "依赖关系不用画图，文字说清楚就行" | 依赖关系必须用 Mermaid 图，文字容易遗漏循环依赖 |
| "Mock 对接任务以后再说" | Mock 存在时必须有对接任务形成闭环，不能留尾巴 |
| "通俗解释是装饰，可有可无" | 通俗解释是帮助理解任务价值的关键字段，必须写 |
| "UI 任务的验证标准不用提原型" | 四份文档驱动模式下，UI 任务必须包含原型一致性检查点 |
| "原型引用标不标无所谓" | UI 任务必须标注原型文件路径，便于实现和审查时对照 |
